import { useSuspenseQuery } from '@tanstack/react-query';
import {
  getCollaboratorBalance,
  getCollaboratorBalanceSummary,
} from './collaboratorBalanceClient';
import { useAuth } from '@clerk/clerk-react';
import type {
  CollaboratorBalanceSummaryQuery,
  ListCollaboratorBalance,
} from '#/features/collaborator-balance/schemas';

export function useCollaboratorBalanceSummarySuspense(
  params: CollaboratorBalanceSummaryQuery
) {
  const { getToken } = useAuth();
  return useSuspenseQuery({
    queryKey: ['collaborator-balance-summary', params],
    queryFn: async () => {
      const token = await getToken();
      return getCollaboratorBalanceSummary(params, token);
    },
  });
}

export function useCollaboratorBalanceSuspense(
  params: ListCollaboratorBalance
) {
  const { getToken } = useAuth();
  return useSuspenseQuery({
    queryKey: ['collaborator-balance', params],
    queryFn: async () => {
      const token = await getToken();
      return getCollaboratorBalance(params, token);
    },
  });
}
