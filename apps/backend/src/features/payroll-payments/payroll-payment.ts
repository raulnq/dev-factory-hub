...
export const payrollPayments = dbSchema.table(
  'payroll_payments',
  {
  ...
  t => ({
    balanceIdx: index('payroll_payments_balance_idx').on(
      t.collaboratorId,
      t.status,
      t.currency,
      t.paidAt
    ),
    pensionBalanceIdx: index('payroll_payments_pension_balance_idx').on(
      t.collaboratorId,
      t.status,
      t.currency,
      t.pensionPaidAt
